...
//
// Trial       -> 100 -> Helper
// Helper      -> 200 -> Sr Helper
// Sr Helper   -> 300 -> Mod
// Mod         -> 400 -> Sr Mod
// Sr Mod      -> 500 -> Jr Admin
// Jr Admin    -> 600 -> Admin
// Admin       -> 700 -> Co Owner
// Co Owner    -> 800 -> Hp Owner
// Hp Owner    -> 900 -> Owner
//
// بعد الترقية:
// النقاط = 0
...
const STAFF_ROLE_NAMES =
  STAFF_RANKS.map(rank => rank.name);
...
if (fs.existsSync(DB_FILE)) {
  try {
    ...
    // حماية من البيانات القديمة/الناقصة
    db.points = db.points || {};
    db.warns = db.warns || {};
    db.tickets = db.tickets || {};
    db.spam = db.spam || {};
    db.pointCooldown = db.pointCooldown || {};
    db.banUntil = db.banUntil || {};

  } catch (error) {
    console.log(
      "❌ تعذر قراءة database.json:",
      error.message
    );
  }
}
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          // مهم:
          // نرسل اللوحة من نفس الـ interaction
          // بدل channel.send ثم reply
          return interaction.reply({
            embeds: [
              embed
            ],
      ...
      if (
        ...
        // ==================================================
        // صاحب التذكرة؟
        // نتحقق منه قبل أي تعديل على النقاط
        // ==================================================
        ...
        if (
          ...
          // إذا الشخص Owner لا يوجد نظام نقاط
          if (
            isOwnerRank(member)
          ) {
            ...
            return interaction.reply({
              content:
                ...
                `🏆 رتبتك Owner ونظام النقاط متوقف عندك.`,
              ephemeral: true
            });
          }
        ...
        // ==================================================
        //
        // نضع claimedBy قبل await
        // لمنع شخصين من استلام نفس التذكرة
        //
